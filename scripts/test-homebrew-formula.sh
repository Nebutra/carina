...
grep -Fq 'version "0.8.22"' "$formula"
grep -Fq 'https://github.com/Nebutra/carina/releases/download/v0.8.22/' "$formula"
grep -Fq 'on_macos do' "$formula"
grep -Fq 'on_linux do' "$formula"
grep -Fq 'carina-ui' "$formula"
grep -Fq 'carina_0.8.22_darwin_arm64.tar.gz' "$formula"
grep -Fq 'carina_0.8.22_darwin_amd64.tar.gz' "$formula"
grep -Fq 'carina_0.8.22_linux_arm64.tar.gz' "$formula"
grep -Fq 'carina_0.8.22_linux_amd64.tar.gz' "$formula"
...
"$ROOT/scripts/check-homebrew-version.sh" "" 0.8.22
"$ROOT/scripts/check-homebrew-version.sh" 0.8.22 0.8.22
"$ROOT/scripts/check-homebrew-version.sh" 0.7.0 0.8.22
if "$ROOT/scripts/check-homebrew-version.sh" 0.9.0 0.8.22 >/dev/null 2>&1; then
