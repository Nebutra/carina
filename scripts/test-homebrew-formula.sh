...
grep -Fq 'version "0.8.4"' "$formula"
grep -Fq 'https://github.com/Nebutra/carina/releases/download/v0.8.4/' "$formula"
grep -Fq 'on_macos do' "$formula"
grep -Fq 'on_linux do' "$formula"
grep -Fq 'carina-ui' "$formula"
grep -Fq 'carina_0.8.4_darwin_arm64.tar.gz' "$formula"
grep -Fq 'carina_0.8.4_darwin_amd64.tar.gz' "$formula"
grep -Fq 'carina_0.8.4_linux_arm64.tar.gz' "$formula"
grep -Fq 'carina_0.8.4_linux_amd64.tar.gz' "$formula"
...
"$ROOT/scripts/check-homebrew-version.sh" "" 0.8.4
"$ROOT/scripts/check-homebrew-version.sh" 0.8.4 0.8.4
"$ROOT/scripts/check-homebrew-version.sh" 0.7.0 0.8.4
if "$ROOT/scripts/check-homebrew-version.sh" 0.9.0 0.8.4 >/dev/null 2>&1; then
