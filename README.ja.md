...
- **Durable job controls**：lineage-scoped な background handle、wait、cancel、summary-only result。
- **Read-only repository evidence**：typed で bounded な Git status、diff、log。
- **Governed browser automation**：隔離された native Chromium、構造化 snapshot、typed action、quarantine artifact。
- **Default-off Proactive**：read-only evidence、canary control、foreground overhead の計測。
...
- Windows arm64/x64 worker、VS Code VSIX、Web Harness archive、macOS/Linux native Tauri Harness bundle；
- `Nebutra/tap/carina` Homebrew Formula。

残る activation と alpha limitation：

- public container registry publication；
- package 済み VS Code/Web Harness client の Marketplace/hosting 公開；
...
全 artifact は [`v0.10.0` release](https://github.com/Nebutra/carina/releases/tag/v0.10.0) から取得できます。
