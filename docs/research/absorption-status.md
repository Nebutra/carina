...
- [x] **OS syscall sandbox** (`carina-run --sandbox`): macOS `sandbox-exec`/SBPL profile confining file writes to the workspace (realpath'd cwd) + `/tmp`, Linux `bwrap` with the same write set. Missing helper is fail-closed (no silent unwrap). Wired through `toolchain.Run` + `Options.SandboxCommands`; `daemon.doctor` reports requested/available/applied. (Linux landlock/seccomp remains a later platform item.)
...
- [x] **Multi-tier compaction** (verbatim-user preservation + v2 receipt +
...
- [x] **Managed-locked config keys** (`go/config/managed.go`,
...
- [x] **Tri-level extension enable-merge** (safe_mode > org > project > user;
...
- [x] **Versioned state stamping + quarantine-not-delete** (new leaf
...
- [x] **MCP-scoped tool search** (`mcp_find`: stateless weighted token-overlap
...
- [x] **MediaRef image plumbing** (`go/daemon/media.go` + `Observation.MediaRefs`;
