...
## ReAct 原理
Thought(推理)→ Action(动作)→ Observation(环境反馈)交错循环。**接地压过幻觉**:每步 Thought 被真实 Observation grounding,错误在下轮被环境纠正而非累积。ReAct 在 ALFWorld/WebShop 比基线 +34%/+10%。pi-os 现有 loop 正是这个骨架(model 出 JSON action,kernel 执行喂回 observation,thought 字段承载推理)。
...
- **并行工具**:只读(read/search/list)并发,变更(patch/run)串行 → 保事务可回滚。pi-os 一轮一动作提速的最大杠杆。
...
- **防无限循环**:动作指纹去重 + 无进展检测(三家都不完善,pi-os 该补)。
...
## → pi-os 7 改进(读了 agent.go 现状:maxAgentTurns=14,strings.Builder 全量拼 history,Think 出错即死,无压缩/并行/循环检测)
