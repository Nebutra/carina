...
**对 pi-os 的直接启示:** pi-os 的**独立 session**(独立 permission profile + 独立 event log + 独立 workspace 边界)天生就是 subagent 隔离的载体。subagent = 一个受限 profile 的子 session + 独立 agent loop,结果回传父 session。
...
**对 pi-os 的启示:** goal 机制 = task 加 `plan`/`success_criteria` + 一个 `read-only`(或 `plan`)profile 做探索阶段,再切到 `safe-edit` 执行。approval_mode 映射到 kernel 的 approval。
...
**对 pi-os 的启示:** agent loop 现在的 transcript 是无限拼接的。生产级需要:event log/transcript 超阈值时触发 compaction(summarize 旧轮次),用同一个 reasoner 或更便宜的做摘要。
...
pi 用「独立进程 + markdown agent + chain/parallel 工具」实现,pi-os 用「独立 session + kernel 受限 profile + scheduler 编排」实现会更安全、可审计(每个 subagent 的每步副作用都进 kernel + 哈希链审计)。
