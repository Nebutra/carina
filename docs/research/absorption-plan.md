...
- [ ] Versioned idempotent config/state migration — [S/additive] (no `STATE_VERSION`-style schema migration ladder found in `go/config`/`go/daemon` session or run stores as of this reconciliation — genuinely open, not just differently named; benchmarked against codex/Claude Code 2026-07-12 — `codex-claudecode-benchmark.md`, verdict `design_only`: reject the generic ladder both a naive reading of Codex and the seed implied, adopt Claude Code's idempotent value-inspecting upgrade pattern plus quarantine-not-delete on version mismatch instead)
