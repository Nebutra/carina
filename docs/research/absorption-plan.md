...
- [ ] Context-aware dynamic skill prompts — [M/additive] (split 2026-07-12 from the bundled line above; the skill-injection half remains open: context-triggered skill/command prompt composition into the stable prompt segment is prompt-assembly work in `agent.go`/`memory.go`, distinct from the image capability and never part of the `content_block_images` design record — see `codex-claudecode-benchmark.md`)
