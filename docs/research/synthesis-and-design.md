...
四路一手调研(见同目录 claude-code-subagent / workflow-orchestration / coding-agent-loop / codex-goal + pi-extensions-findings)收敛出的统一设计。

## 统一洞察:为什么 pi-os 是这些机制的理想基座

四份调研反复指向同一结论 —— pi-os 已有的四个底牌,恰好是这些机制最安全的落地基础,而且能让 pi-os 版本**比原版更可控、可审计**:
...
- **两轴解耦**:Task 加 `approval_mode`(untrusted/on_request/never),从 Profile 拆出。`pi-policy.evaluate` 加 approval_mode 入参:never 把 RequiresApproval 降 Allowed(profile 兜底);untrusted 把非白名单命令升 RequiresApproval。
...
- **AgentSpec**:markdown + frontmatter(name/description/tools→capability grants/model/max_turns),从 `~/.pi-os/agents/*.md` + `.pi-os/agents/*.md` 加载。
