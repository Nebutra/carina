...
> 本文是立项时的 point-in-time PRD，保留作历史记录，不随实现更新。当前架构见 `docs/architecture.md`，实现吸收状态见 `docs/research/absorption-status.md` 与 `docs/roadmap.md`。
...
> 实现形态见 `docs/worker-executor.md`：远程 worker 使用独立的 `carina-worker` 二进制与 `wss://` Gateway，`--server` 仅限 loopback。
...
**验收**:企业可内部部署 Carina Runtime,并对所有 Agent 行为进行审计和限制。
...
本项目不是"重写 Carina",而是把 Carina 从 **LLM CLI Tool** 升级为 **Agent OS Runtime**。
