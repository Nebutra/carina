...
| P5 | run token ceiling、完成/失败/跳过/running/queued 聚合、channel activity、durable operator detail、CLI foreground/background/status/control | 本地/remote rollup、动态节点进入 detail、stop 真取消、pause 仅冻结新节点 | remote result v1 不含 token，明确显示 `unmetered_steps`/`observed_usage_only`，不能伪称为 0 或完整预算 |
