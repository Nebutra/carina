...
状态：设计提案（未实现）。承接 `docs/research/workflow-orchestration.md` 的图模型词汇，专门解决它没覆盖的三件事：**万级并发规模的执行语义**、**运行时活体通信**、**跨机器集群分发**。
...
| **内核/审计吞吐（最关键瓶颈）** | 每次 capability 请求：daemon→kernel 子进程同步 JSON-RPC→`Mutex<AuditState>`→同步文件 append | `go/kernel/kernel.go:237-247`；`crates/carina-audit/src/lib.rs:200,236-257` | **单一互斥锁+同步文件 I/O 串行化全 daemon 所有 agent 的所有副作用请求**——这是任何"加线程/加节点"方案会立刻撞上的真正天花板，不解决这个，其它都是空中楼阁 |
...
**核心决策**：不是让一个 kernel 服务撑住上千并发请求，而是**每个 worker 节点跑自己完整的 daemon+kernel+audit 栈**，coordinator 只做图调度和跨节点消息路由。这直接解决了吞吐瓶颈（N 个独立 audit mutex 而不是 1 个），也天然对应"集群式"的要求，还保持治理不打折（每台机器的每个副作用仍然被它自己的 kernel 完整网关+审计，不存在"coordinator 说了算、本地不检查"的旁路）。
...
## 8. 内核/审计吞吐：为什么"每节点一个 daemon"就够了

这是本设计对现有瓶颈最直接的回应。单个 daemon 内，`crates/carina-audit/src/lib.rs:200` 的 `Mutex<AuditState>` 序列化该 daemon 处理的**所有**会话的审计写入——这是 per-daemon 的锁，不是 per-session 的。把 1000 个 agent 塞进一个 daemon 无论怎么加并发度都会在这里撞墙。

方案不是"让这把锁变快"（虽然 §9 提到的批量 fsync 仍值得做，作为独立优化），而是**架构上不让 1000 个 agent 挤进一个 daemon**：swarm 按 §7 分摊到 N 台机器上的 N 个 daemon，每个 daemon 本地仍然只服务它承接的那部分节点（现有的每 daemon 64-256 并发量级完全够用），核销瓶颈的方式是水平扩展 daemon 数量,而不是纵向拉伸单个 kernel 的吞吐上限。这与"集群式"的要求是同一个答案:分布式不只是为了跨机器算力，也是为了绕开单进程治理层的物理串行化限制,同时不牺牲治理本身。
...
| 阶段 | 内容 | 依赖 | 验收标志 |
|---|---|---|---|
| **P1** | 单 daemon 内把 `workflow.go` 的按层 barrier 换成流式 ready-queue + 有界 worker 池；`maxWorkflowSteps` 从编译期常量改运行时可配置上限；`on_failure: isolate` 默认化 | 无（纯本地改造现有代码） | 单 daemon 内跑通 200+ 节点、异构耗时的图，无 barrier 空等 |
| **P2** | 结构化 `input`/`output`（data edge）+ 条件边（沙箱表达式）+ generator 动态节点注入，节点数/深度安全上限 | P1 | 图可以在运行中长出新节点，仍受硬顶约束 |
| **P3** | `SwarmMessage` channel 通信，复用 `go/channels` 信任模型 | P1 | 节点间可在运行中互相推送消息，不必等上游完成 |
| **P4** | Remote worker 真正实现（填 `go/worker` 的 Remote kind 空白）+ `RemoteDispatch` 能力 + coordinator/worker 分离 | P1-P3（图/通信语义先在单机跑稳，再谈跨机） | 一次 swarm 运行真实跨 2+ 台机器分摊执行 |
| **P5** | 预算树聚合视图 + 大规模可观测性聚合层 | P1-P4 | 1000 节点规模的 swarm 运行，coordinator 侧内存/事件量不随节点数线性爆炸给单一订阅者 |

P1 单独就有独立价值（现有 workflow 引擎的 barrier 问题不需要等集群能力就能修），可以先落地验证调度语义，再决定是否投入 P3/P4 这类工作量更大的部分。

## 13. 关键风险与待定问题

- **审计吞吐的批量优化仍值得做**：即使按 §8 水平扩展 daemon 数量，单 daemon 内 group-commit 式批量 fsync（多个 capability 决策合并进一次文件 write，哈希链在内存里仍顺序计算）能再抬高单机上限，是否值得投入取决于 P1 跑出来的真实数字，不是本设计的阻塞项。
- **Coordinator 单点**：设计里 coordinator 是"逻辑单例"，第一阶段可以是真单点（配合现有 event-log 可重放的哲学做崩溃恢复），双活/多活是后续问题，不在 P1-P5 范围内强求。
- **条件表达式引擎选型**（CEL vs JSONLogic）：`workflow-orchestration.md` 已经提出这个问题，本设计沿用其结论待定，不重复决策——两个方案都满足"沙箱、无可执行代码"的硬约束,选型应该在两份文档共同的图 schema 定稿时一次性拍板,避免两套引擎。
- **generator 动态节点的滥用风险**：一个恶意/失控的 generator 节点理论上可以持续生成节点直到撞上硬顶,这本质是"图规模的 DoS"。硬顶+审批型 `SwarmSpawn` 能力請求在图规模超过阈值时重新触发审批,是本设计给出的缓解,但生产化前需要更仔细的速率限制设计（如"每分钟最多注入 N 个节点")。
- **本设计与 `workflow-orchestration.md` 的关系需要在两份文档定稿时合并成一份权威图 schema 参考**,当前两份文档并存是因为设计视角不同(BSP 正确性优先 vs 流式规模优先),但工程实现不应该分裂成两套互不兼容的图执行引擎——目标是"同一图 schema,两种可选执行模式(`execution_mode: bsp | streaming`)",而不是两个产品。
