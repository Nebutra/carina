...
**核心决策**：不是让一个 kernel 服务撑住上千并发请求，而是**每个 worker 节点跑自己完整的 daemon+kernel+audit 栈**，coordinator 只做图调度和跨节点消息路由。目标拓扑用 N 条独立的 kernel service 治理链路分散负载，也天然对应"集群式"的要求，还保持治理不打折（每台机器的每个副作用仍然被它自己的 kernel 完整网关+审计，不存在"coordinator 说了算、本地不检查"的旁路）。当前 as-built remote worker 仍是 operator-supplied executor，并不自动拥有完整本地 kernel，边界见 §3 图前说明。
...
- **审计 durability/句柄开销——已收口；吞吐优化仍待数据（2026-07-12）**：`AuditLog` 现在长期持有文件句柄，每次 append 后 `sync_all()`；Unix 上每次打开还同步父目录，覆盖主机崩溃/断电时目录项丢失的窗口。write/fsync 失败会 poison 当前实例，禁止基于陈旧链头继续写；并发压力和故障注入测试覆盖链完整性与失败关闭。代价是每事件 fsync 会增加延迟，并可能放大单线程 kernel service 的跨 session 头阻塞。group commit 或并行 kernel 请求处理都必须先以真实吞吐/延迟基准证明收益，再设计生命周期和顺序语义；因此吞吐不是“已经收口”，而是明确保留的性能 TODO。
