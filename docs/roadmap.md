...
| Screen mode and terminal buffer | Fullscreen is the default alt-screen product surface (mouse capture, no shell scroll-out); Minimal is the opt-in commit-once native-scrollback projection; Inline disables mouse/focus/graphics/native commit as a keyboard-safe fallback. `/minimal`, `/fullscreen`, `/inline`, and `--screen-mode` re-exec with a fenced session/runtime handoff | Ledger watermark regressions prove zero duplicate commit across reattach; capability-matrix, launcher, handoff identity/governance/selection, render geometry, and PTY terminal-family tests cover the three modes |
| Locale UX and microcopy | Complete en, zh (Simplified), zh-Hant (Traditional, derived), ja, ko, es, and fr catalogs cover interface, Ambient, Governed, Degrade, and bootstrap copy; locale precedence, explicit-value validation, safe placeholder rendering, CLDR count selection, facts/terms metadata, and catalog parity are repository contracts | CI tests traverse all locales (including zh-Hant), placeholders, register lint, count categories, fallback behavior, startup help/errors, and catalog versions. Traditional is OpenCC-derived from Simplified (`scripts/gen_zh_hant.py`); CI `quality-guardrails` fails on a stale generated table (`make zh-hant-check`) |
...
| Side Q&A and session fork | `/btw` answer-only on the active run; `/btw --fork` and `/side` withdrawn (use `/fork`) | TUI registry + `execution.btw` RPC tests |
| Plan line comments | `/view-plan` overlay: `c` comment, `m` mark range; request-changes seeds line notes | Plan review unit tests |
| Product HITL modes | Daemon modes `ask` \| `always-approve` \| `dont-ask` \| `accept-edits`; named presets `read-only` / `agent` / `accept-edits` are labels only; `/always-approve` warns and is not a cycle preset; org `disable_always_approve`; footer shows mode; plan review overlay via `/view-plan` | Daemon resolve-approval tests, RPC mode tests, config validation, accept-edits + plan-review TUI tests; session/kernel axis `untrusted\|on_request\|never` remains separate |
