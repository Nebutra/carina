...
## Roles (see `docs/brand/AGENTS.md` and `design-system/DESIGN.md`)

| Role | Family | Shipped here |
| --- | --- | --- |
| Brand identity | **Carina Display Alpha** | Yes (TTF / WOFF2 + SVG glyphs) |
| Editorial titles | **Newsreader** | No — docs site loads via `@fontsource-variable/newsreader` (or equivalent licensed source) |
| UI / body / H2+ | **Geist Sans** | Yes (variable WOFF2) |
| Mono / attested data | **Geist Mono** | Yes (variable WOFF2) |

## Carina Display Alpha

Experimental brand-display font derived from the accepted CARINA wordmark.

- Use for the Carina name and composed identity lockups only.
- Do not use for body copy, controls, code, audit data, CLI/TUI output, documentation H1, or accessibility-critical text.
...
Approved **editorial** face for documentation and marketing primary titles (page H1, hero).

- Not a wordmark font — never typeset `CARINA` in Newsreader to replace the lockup SVG.
- Not for body, navigation, controls, or code.
- Licensed separately (OFL via fontsource / Google Fonts distribution); not bundled in this directory.
