...
var updateCatalogRows = []catalogRow{
	...
	catalog(MsgExplainApprovalModes, "Modes: ask | always-approve | dont-ask (deny without grant; good for CI). Set with /approval-mode or /dont-ask. Org may set disable_always_approve.", "模式：ask | always-approve | dont-ask（无授权则拒绝，适合 CI）。/approval-mode 或 /dont-ask。组织可设 disable_always_approve。", "モード: ask | always-approve | dont-ask（grant なし拒否・CI 向け）。/approval-mode または /dont-ask。組織は disable_always_approve 可。", "모드: ask | always-approve | dont-ask(grant 없으면 거부, CI용). /approval-mode 또는 /dont-ask. org는 disable_always_approve 가능.", "Modos: ask | always-approve | dont-ask (sin grant se deniega; CI). /approval-mode o /dont-ask. Org puede bloquear always-approve.", "Modes : ask | always-approve | dont-ask (refus sans grant ; CI). /approval-mode ou /dont-ask. Org peut verrouiller always-approve."),
