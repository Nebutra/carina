...
var updateCatalogRows = []catalogRow{
	...
	catalog(MsgUpdateUsageApprovalMode, "usage: /approval-mode [ask|always-approve|dont-ask]", "用法：/approval-mode [ask|always-approve|dont-ask]", "使用法: /approval-mode [ask|always-approve|dont-ask]", "사용법: /approval-mode [ask|always-approve|dont-ask]", "uso: /approval-mode [ask|always-approve|dont-ask]", "utilisation : /approval-mode [ask|always-approve|dont-ask]"),
	...
	catalog(MsgExplainApprovalModes, "Product HITL: ask | always-approve | dont-ask (/approval-mode, /dont-ask). Org may set disable_always_approve. Separate session/kernel axis: untrusted|on_request|never on session create — not product approval_mode.", "产品 HITL：ask | always-approve | dont-ask（/approval-mode、/dont-ask）。组织可设 disable_always_approve。会话/内核轴 untrusted|on_request|never 在 session 创建时设置，不是产品 approval_mode。", "製品 HITL: ask | always-approve | dont-ask。組織は disable_always_approve 可。セッション/kernel 軸 untrusted|on_request|never は session 作成時 — 製品 approval_mode ではない。", "제품 HITL: ask | always-approve | dont-ask. org는 disable_always_approve 가능. 세션/kernel 축 untrusted|on_request|never는 세션 생성용 — 제품 approval_mode 아님.", "HITL de producto: ask | always-approve | dont-ask. Org puede disable_always_approve. Eje sesión/kernel untrusted|on_request|never al crear sesión — no es approval_mode de producto.", "HITL produit : ask | always-approve | dont-ask. Org : disable_always_approve. Axe session/kernel untrusted|on_request|never à la création — pas le approval_mode produit."),
