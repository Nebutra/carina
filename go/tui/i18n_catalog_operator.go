...
var operatorCatalogRows = []catalogRow{
	...
	catalog(MsgCheckpointNoRecent, "{glyph} no recent restored task is known; use /resume <task_id> after restarting the TUI", "{glyph} 没有已知的近期恢复任务；重启 TUI 后请使用 /resume <task_id>", "{glyph} 最近の復元タスクがありません。TUI 再起動後は /resume <task_id> を使用", "{glyph} 최근 복원된 작업이 없습니다. TUI 재시작 후 /resume <task_id>를 사용하세요", "{glyph} no se conoce una tarea restaurada reciente; usa /resume <task_id> tras reiniciar", "{glyph} aucune tâche restaurée récente ; utilisez /resume <task_id> après redémarrage"),
