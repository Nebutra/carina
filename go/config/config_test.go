...
func TestCascadePrecedence(t *testing.T) {
	...
	writeConfig(t, home, `{"offline": true, "max_task_tokens": 100, "tools_dir": "/g/tools", "gateway_http": "127.0.0.1:7000", "gateway_http_origins": ["https://api.example"], "gateway_ws": "127.0.0.1:7001", "gateway_ws_origins": ["https://app.example"], "gateway_token_signing_key_file": "/g/token.key", "gateway_token_max_ttl_seconds": 600, "enable_debug_rpc": true, "summarizer_model": "cheap", "risk_review_model": "guardian", "context_engine": "noop"}`)
	...
	t.Setenv("CARINA_CONTEXT_ENGINE", "off")
...
func TestInspectTUIAlternateScreenIgnoresInvalidLocale(t *testing.T) {
	scrubCarinaEnv(t)
	home := t.TempDir()
	project := t.TempDir()
	writeConfig(t, home, `{"tui_locale":"invalid","tui_alternate_screen":"never"}`)
	mode, err := InspectTUIAlternateScreen(home, project)
	if err != nil {
		t.Fatal(err)
	}
	if mode != "never" {
		t.Fatalf("alternate screen mode = %q, want never", mode)
	}
}
