...
fn main() {
    ...
    if args.len() != 3 {
        eprintln!("usage: pi-wat2wasm <input.wat> <output.wasm>");
        exit(2);
    }
    let source = match std::fs::read_to_string(&args[1]) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("pi-wat2wasm: read {}: {e}", args[1]);
            exit(1);
        }
    };
    let wasm = match wat::parse_str(&source) {
        Ok(bytes) => bytes,
        Err(e) => {
            eprintln!("pi-wat2wasm: compile error: {e}");
            exit(1);
        }
    };
    if let Err(e) = std::fs::write(&args[2], &wasm) {
        eprintln!("pi-wat2wasm: write {}: {e}", args[2]);
        exit(1);
    }
