...
| Local plugin inventory | **wired** | `/plugins` projects `extension.list` (name, source, caps, enabled); safe-mode empty reason; TUI never calls `plugin.run` | ISSUE-027 / #36 |
...
| Subagent observation surface | **wired** | `agent.view` / `agent.recap`; `/agents` opens the retained read-only roster (id, spec, status, profile, worktree); recap stays in the overlay | ISSUE-012 / #35 |
...
| Dual-axis HITL state | **wired** | typed `config.inventory`; named presets `read-only` / `agent` / `accept-edits` label existing product mode × profile pairs; `/always-approve` stays an explicit verb; no yolo preset | ISSUE-010 / #37 |
...
| Context ledger = model-visible prompt | **wired** | `context.summary.ledger` projects `tr.render()`, prompt layers, elided/pinned turns, receipts; prefix cache is `anthropic` only on Messages-API routes, else `cache=none` (Grok/JSON-only, OpenAI, unknown); `/context` overlay consumes the RPC only | ISSUE-019 / #28 / #34 |
| Compact as first-class cell | **wired** | `ContextCompacted` with a receipt becomes `compact:{event_id}` live=replay (`context_compacted` items); request-only and identity-engine events stay off the transcript; `engine=noop` never claims savings | G23-03 |
| Tool-result snip on enqueue | **wired** | oversized observations persist in `go/artifact`; model view keeps head/tail + sha256 pointer; pinned stays full; `CARINA_INLINE_TOOL_OUTPUT=1` restores inline | ISSUE-020 / #29 |
