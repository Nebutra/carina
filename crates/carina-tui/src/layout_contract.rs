...
pub const TRANSCRIPT_ROLE_GAP_WIDTH: usize = 1;
...
/// First-line role prefix width for a already-truncated label.
pub fn transcript_role_first_width(label_cells: usize) -> usize {
    TRANSCRIPT_ROLE_MARK_WIDTH + label_cells + TRANSCRIPT_ROLE_GAP_WIDTH
}
...
mod tests {
    ...
    fn transcript_indents_are_relational_and_width_locked() {
        assert_eq!(TRANSCRIPT_ROLE_PREFIX_WIDTH, 9);
        assert_eq!(transcript_role_first_width(2), 5);
        assert_eq!(transcript_role_first_width(3), 6);
        assert_eq!(transcript_role_first_width(6), 9);
