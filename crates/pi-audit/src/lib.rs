//! Append-only, tamper-evident event log for Pi-OS (PRD §8.2, §9.2).
//!
...
impl AuditLog {
    ...
    /// (PRD §9.2: `pi audit verify` must detect tampering).
...
mod tests {
    ...
    fn tmp(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("pi-audit-{name}-{}-{n:x}", std::process::id()))
    }
