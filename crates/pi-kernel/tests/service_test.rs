//! Integration tests that drive `pi-kernel-service` over stdio exactly as
...
impl Service {
    fn start(state_dir: &std::path::Path) -> Self {
        ...
        let mut child = Command::new(bin)
            ...
            .spawn()
            .expect("spawn pi-kernel-service");
...
fn setup() -> (tempWorkspace, Service) {
    let base = std::env::temp_dir().join(format!("pi-kernel-it-{}-{}", std::process::id(), rand_suffix()));
    let ws = base.join("ws");
