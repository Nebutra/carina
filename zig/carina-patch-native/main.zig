//! carina-patch-native — atomic multi-file patch primitive (PRD §8.5, §4.4).
//!
//! This is the real local-system write path: the Rust transactional engine
//! (crates/carina-patch) owns the lifecycle state machine and authorization,
...
//!
//! Subcommands:
//!   carina-patch-native apply     write every file atomically (temp + rename);
//!                             all-or-nothing — any failure restores prior files
//!   carina-patch-native dry-run   validate only; touch nothing
//!   carina-patch-native rollback  restore each file from its snapshot (or delete
...
pub fn main() !void {
    ...
    if (args.len < 2) {
        try jsonl.writeLine("{\"error\":\"usage: carina-patch-native <apply|dry-run|rollback>  (JSON plan on stdin)\"}");
        std.process.exit(2);
    }
    const cmd = args[1];
...
fn atomicWrite(allocator: std.mem.Allocator, path: []const u8, content: []const u8) !void {
    ...
    const tmp = try std.fmt.allocPrint(allocator, "{s}.carina-tmp", .{path});
