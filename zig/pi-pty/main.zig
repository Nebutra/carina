//! pi-pty — real interactive pseudo-terminal (PRD §8.5, §8.7).
//!
...
//!
//! Usage:
//!   pi-pty [--cols N] [--rows N] [--timeout-ms N] [--json] -- <program> [args...]
//!
...
pub fn main() !void {
    ...
    if (argv_start == 0 or argv_start >= args.len) {
        try jsonl.writeLine("{\"error\":\"usage: pi-pty [--cols N] [--rows N] [--timeout-ms N] [--json] -- <program> [args...]\"}");
        std.process.exit(2);
    }
