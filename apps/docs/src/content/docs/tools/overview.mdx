---
title: Tools overview
description: Built-in agent tools, Zig toolchain, and capability mediation.
sidebar:
  order: 1
---

import CategoryTag from '../../../components/ui/CategoryTag.astro';
import Tree from '../../../components/mdx/Tree.astro';
import Tooltip from '../../../components/mdx/Tooltip.astro';
import Columns from '../../../components/mdx/Columns.astro';
import Card from '../../../components/mdx/Card.astro';

<CategoryTag category="tool" label="Tools" />

Tools are the only path from model intent to system effect. Carina normalizes tool calls so <Tooltip tip="Every tool call is a capability check + audit event before Zig executes.">policy, audit, and UI</Tooltip> treat them uniformly.

## Toolchain layout

<Tree
  items={[
    {
      name: 'bin/',
      children: [
        { name: 'carina-scan' },
        { name: 'carina-grep' },
        { name: 'carina-diff' },
        { name: 'carina-patch-native' },
        { name: 'carina-run' },
        { name: 'carina-pty' },
      ],
    },
  ]}
/>

<Columns cols={2}>
  <Card title="Filesystem" icon="📂">
    list · read · search · patch — always workspace-scoped.
  </Card>
  <Card title="Shell" icon="⌘">
    `run` via risk-classified CommandExec + allowlists.
  </Card>
</Columns>

## Built-in agent tools

See the full action table in [Agents overview](/agents/overview/). Families:

- Filesystem: list, read, search, edit (exact unique span), patch
- Git information: `git.status`, `git.diff`, `git.log` (typed, bounded, read-only)
- Network: `web.fetch` (public HTTPS, host approval)
- Browser: `browser.open`, `browser.snapshot`, `browser.action`, `browser.tabs`, `browser.capture`, `browser.close`
- Shell: `run` (risk-classified `CommandExec`)
- Memory: governed long-term entries
- Code intelligence: search / symbols / map / def / refs / impact
- MCP: external servers via governed manager
- Orchestration: spawn, workflow, best_of_n
- Human: ask_user

## Read-only Git evidence

| Tool | Result |
| --- | --- |
| `git.status` | Branch, upstream, ahead/behind counts, and bounded porcelain changes |
| `git.diff` | Bounded per-file patches for an explicit `worktree`, `staged`, or `head` view |
| `git.log` | Structured HEAD history with bounded commit and subject counts |

All three require `FileRead`, run only at the session workspace's repository
root, and accept typed fields rather than raw Git arguments. Prompts, optional
locks, pagers, hooks, fsmonitor, executable diff/filter helpers, credentials,
and submodule recursion are disabled. Binary and untracked content is reported
as metadata instead of being opened or injected into model context.

## Governed browser computer

| Tool | Result |
| --- | --- |
| `browser.open` | Opens an isolated managed browser, or navigates an existing tab to an approved public HTTPS target |
| `browser.snapshot` | Returns a bounded accessibility/DOM projection with opaque, generation-scoped refs |
| `browser.action` | Performs one typed click, type, select, check, key, scroll, hover, upload, or dialog action |
| `browser.tabs` | Lists, opens, activates, or closes tabs without exposing raw CDP |
| `browser.capture` | Stores a bounded screenshot as a session-scoped artifact and media ref |
| `browser.close` | Stops the browser and removes its ephemeral profile |

Managed mode uses a private, ephemeral Chromium profile and does not inherit
the operator's cookies, extensions, password store, or local browser state.
Every navigation and request origin is constrained to approved public HTTPS;
page text, titles, dialogs, downloads, and screenshot pixels remain untrusted.
Element refs become stale after document changes, so actions require a fresh
snapshot instead of raw selectors or coordinates.

Uploads require `FileRead` for each explicit source and a live approval at the
transmission boundary. Downloads are quarantined as artifacts and are never
opened, executed, or written into the workspace. High-impact actions pause
immediately before the browser effect even under `always-approve`. Attaching an
operator browser is a separate deployment-configured mode that always requires
live `BrowserAttach` approval and never exposes its debugging endpoint to the
model. See [Local install](/deployment/local/) for operator configuration.

## Zig native toolchain

| Binary | Role |
| --- | --- |
| `carina-scan` | Workspace file tree |
| `carina-grep` | Structured search |
| `carina-diff` | Structured diff |
| `carina-patch-native` | Apply / verify / rollback / dry-run |
| `carina-run` | Command execution with timeout / env allowlist |
| `carina-pty` | Interactive terminal sessions |

These emit machine-readable JSON and **never** bypass kernel policy.

## Code intelligence

| Tool | Result |
| --- | --- |
| `code.search` | Ranked keyword and semantic matches |
| `code.symbols` / `code.def` / `code.refs` | Definitions, references, and precision provenance |
| `code.impact` | Governed transitive dependents |
| `code.map` | A token-bounded projection of the workspace symbol graph |

Large workspaces do not block the agent on a first-use full scan. Carina builds
one resumable graph per workspace in bounded background batches. While it is
building, `code.map` returns explicit progress and the best available partial
projection; a metadata-only overview is used until the first semantic batch is
ready. Completed graphs are reused across daemon restarts when the persisted
source fingerprint still matches the workspace.

```text frame="none"
Index coverage: complete; graph: 6434 indexed files, 44120 symbols, 228788 edges, 82687 chunks;
projection: 37/5689 files and 37/44120 symbols (pagerank-domain-diverse, about 1017 tokens).
```

The finite projection is not presented as an exhaustive dump. PageRank provides
global relevance; selection covers top-level domains and ranked files before
repeating a file. Each retained symbol carries a global rank plus incoming and
outgoing edge counts. Ingestion, status, and map reads all remain gated by the
session's `CodeIndex` and `FileRead` policy.

## Safety rules

1. Every tool has a capability requirement
2. Arguments are validated before execution
3. Outcomes (success, denial, error) are audit events
4. Plugins and tools carry manifests; undeclared capability use → `PolicyViolation`

## Source of truth

- Agent tool table + Zig native bins (`carina-scan`, `carina-grep`, `carina-patch-native`, …)
- Capability requirements: [Policy](/concepts/policy/) · `protocol/capabilities/`
- Related: [MCP](/tools/mcp/) · [Agents](/agents/overview/) · [CLI reference](/reference/cli/)

## Next

- [MCP](/tools/mcp/) — external tools still policy-gated
- [Audit](/concepts/audit/) — every tool outcome is an event
