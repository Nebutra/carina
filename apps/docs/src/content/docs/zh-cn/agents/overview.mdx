...
| `git.status` / `git.diff` / `git.log` | `FileRead` | 有上限的只读仓库证据；类型化视图，不接受原始 Git 参数 |
| `run` | `CommandExec` + 风险分级 | 受超时和环境变量白名单约束的命令结果 |
| `edit` | `PatchApply` | 精确唯一片段替换，走同一套补丁事务 |
| `patch` | `PatchApply` | 可验证、可回滚的补丁事务 |
| `web.fetch` | `NetworkAccess` | 仅公开 HTTPS；按主机审批；不跟随重定向 |
| `browser.open` / `snapshot` / `tabs` / `capture` / `close` | `BrowserInteract`；导航另需 `NetworkAccess` | 隔离的原生 Chromium 生命周期、有上限的快照和 artifact |
| `browser.action` | `BrowserInteract`；上传另需 `FileRead` | 单次类型化 ref 动作；高影响效果前实时审批 |
...
| 后端 | 用途 |
| --- | --- |
| **model-router** | BYOK provider、模型路由、流式输出与 token 统计。稳定前缀按 constitution / workspace / catalog 分段缓存。目录声明 native tools 的模型先走 HTTP 工具，失败再回 JSON。`grok-build/*` 只走隔离 `grok` CLI 的 JSON ReAct。 |
