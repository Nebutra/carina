---
title: Tools 概览
description: 由能力内核中介的内置工具、Zig 工具链与外部工具。
---

import CategoryTag from '../../../../components/ui/CategoryTag.astro';
import Tree from '../../../../components/mdx/Tree.astro';
import Callout from '../../../../components/ui/Callout.astro';

<CategoryTag category="tool" label="Tools" />

工具是模型意图到系统效果的唯一路径。Carina 将工具调用规范化，因此策略、审计、TUI 与 SDK 对同一动作看到一致的生命周期。

## 工具族

| 工具族 | 典型动作 | 治理边界 |
| --- | --- | --- |
| 文件系统 | list / read / search / edit / patch | 只能访问会话工作区；`edit` 是精确唯一片段 |
| Git 信息 | `git.status` / `git.diff` / `git.log` | 类型化、有上限、只读，并受 `FileRead` 约束 |
| 网络 | `web.fetch` | 公开 HTTPS、按主机审批、不跟随重定向 |
| 浏览器 | `browser.open` / `snapshot` / `action` / `tabs` / `capture` / `close` | 隔离 Chromium、类型化动作、逐来源网络授权与产物隔离 |
| Shell | `run` | `CommandExec` 风险分级、超时、环境白名单 |
| Memory | add / replace / remove | `MemoryWrite` 审批与内容扫描 |
| 代码智能 | symbols / def / refs / impact | `FileRead` + 本地索引 |
| MCP | `mcp` / `mcp_find` | 外部服务仍需能力授权 |
| 编排 | spawn / workflow / best_of_n | 子能力衰减与独立审计 |
| 人工输入 | `ask_user` | 结构化 operator decision |

## 只读 Git 证据

| 工具 | 结果 |
| --- | --- |
| `git.status` | 分支、上游、ahead / behind 数量与有上限的 porcelain 变更 |
| `git.diff` | 显式 `worktree`、`staged` 或 `head` 视图下逐文件、有上限的补丁 |
| `git.log` | 结构化 HEAD 历史，并限制提交数量与主题长度 |

三个工具都只在会话工作区对应的仓库根目录运行，不接受原始 Git 参数。
运行时会禁用交互提示、可选锁、pager、hook、fsmonitor、可执行 diff / filter
helper、credential helper 与 submodule 递归。二进制和未跟踪文件只返回元数据，
不会读取内容并注入模型上下文。

## 受治理的浏览器操作

| 工具 | 结果 |
| --- | --- |
| `browser.open` | 打开隔离的托管浏览器，或把已有 tab 导航到获批的公开 HTTPS 目标 |
| `browser.snapshot` | 返回有上限的 accessibility / DOM 投影，以及按文档代次生效的不透明 ref |
| `browser.action` | 执行一次类型化 click、type、select、check、key、scroll、hover、upload 或 dialog 动作 |
| `browser.tabs` | 列出、打开、激活或关闭 tab，不暴露原始 CDP |
| `browser.capture` | 把有上限的截图保存为会话级 artifact 和 media ref |
| `browser.close` | 停止浏览器并删除临时 profile |

托管模式使用私有、临时的 Chromium profile，不继承操作者的 cookie、扩展、
密码库或本机浏览器状态。每个导航与请求 origin 都只能访问获批的公开 HTTPS；
页面文字、标题、dialog、download 元数据和截图像素一律是不可信输入。文档变化后
element ref 会失效，Agent 必须重新 snapshot，不能传原始 selector、坐标或脚本。

上传只接受逐文件通过 `FileRead` 的显式路径，并在真正发送前要求实时审批。
下载会立即隔离成 artifact，不会自动打开、执行或写入工作区。高影响动作即使在
`always-approve` 下也会在浏览器副作用前暂停。附着操作者浏览器是独立的部署配置
模式，始终要求实时 `BrowserAttach` 审批，debugging endpoint 不会暴露给模型。
操作者配置见[本地安装](/zh-cn/deployment/local/)。

## Zig 原生工具链

<Tree
  items={[
    {
      name: 'bin/',
      children: [
        { name: 'carina-scan' },
        { name: 'carina-grep' },
        { name: 'carina-diff' },
        { name: 'carina-patch-native' },
        { name: 'carina-run' },
        { name: 'carina-pty' },
      ],
    },
  ]}
/>

| 二进制 | 职责 |
| --- | --- |
| `carina-scan` | 输出工作区文件树 |
| `carina-grep` | 结构化文本搜索 |
| `carina-diff` | 生成结构化差异 |
| `carina-patch-native` | dry-run、应用、校验与回滚补丁 |
| `carina-run` | 带超时和环境约束的命令执行 |
| `carina-pty` | 交互终端会话 |

这些工具输出机器可读 JSON，但**不会绕过能力内核**。原生工具负责快速、确定地执行；是否允许执行仍由策略决定。

## 代码智能

| 工具 | 结果 |
| --- | --- |
| `code.search` | 排序后的关键词与语义匹配 |
| `code.symbols` / `code.def` / `code.refs` | 定义、引用与精度来源 |
| `code.impact` | 受治理的传递依赖影响范围 |
| `code.map` | 工作区符号图的 token 有界投影 |

大型工作区首次使用时不会让 Agent 阻塞等待完整扫描。Carina 为每个工作区维护一份可恢复的完整图，并在后台按有界批次构建。构建期间，`code.map` 会返回明确进度和当前可用的局部语义投影；首个语义批次就绪前使用元数据概览。daemon 重启后，只有持久化源指纹仍与工作区匹配时才会复用已完成图。

```text frame="none"
Index coverage: complete; graph: 6434 indexed files, 44120 symbols, 228788 edges, 82687 chunks;
projection: 37/5689 files and 37/44120 symbols (pagerank-domain-diverse, about 1017 tokens).
```

有限投影不会伪装成完整符号转储。PageRank 提供全局相关性；选择器先覆盖不同顶层域和已排序文件，再为同一文件分配更多符号。每个保留符号都携带全局排名以及入边、出边数量。摄取、状态检查和图读取仍同时受会话 `CodeIndex` 与 `FileRead` 策略约束。

## 安全规则

1. 每个工具声明所需能力。
2. 参数在执行前校验，路径会归一化到工作区。
3. 成功、拒绝和错误都会进入事件流。
4. 插件若使用 manifest 未声明的能力，会产生 `PolicyViolation`。
5. 写文件优先走 patch，而不是把任意 shell 当编辑器。

<Callout variant="warning">
  “工具已注册”不代表“当前会话可调用”。实际权限取决于 session profile、资源范围和审批结果。
</Callout>

## 权威来源

- Agent 工具表与 Zig 原生工具：`carina-scan`、`carina-grep`、`carina-patch-native` 等
- 能力要求：[策略](/zh-cn/concepts/policy/) · `protocol/capabilities/`
- 相关：[MCP](/zh-cn/tools/mcp/) · [Agents](/zh-cn/agents/overview/) · [CLI 参考](/zh-cn/reference/cli/)

## 下一步

- [MCP](/zh-cn/tools/mcp/) — 外部工具仍过策略
- [审计](/zh-cn/concepts/audit/) — 每次工具结果都是事件
