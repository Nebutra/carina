...
Carina drives a real ReAct coding agent. The model **only decides**; every side effect is authorized by the Rust capability kernel and executed by the Zig toolchain. The whole run is a tamper-evident audit trail you can replay and roll back.
