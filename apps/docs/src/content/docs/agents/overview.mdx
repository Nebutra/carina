...
<Mermaid
  caption="Decide → authorize → execute → observe."
  code={`flowchart LR
...
| `{"tool":"git.status/git.diff/git.log",…}` | FileRead | bounded read-only repository evidence; typed views, no raw Git argv |
| `{"tool":"run","command":["…"]}` | CommandExec (risk-classified) | Zig `carina-run` |
| `{"tool":"edit","path","old","new"}` | PatchApply | exact unique span → same transactional patch as `patch` |
| `{"tool":"patch",…}` | PatchApply | Rust transaction → Zig `carina-patch-native` |
| `{"tool":"web.fetch","url"}` | NetworkAccess | public HTTPS; host approval; no redirects |
| `{"tool":"browser.open/snapshot/tabs/capture/close",…}` | BrowserInteract (+ NetworkAccess for navigation) | isolated native Chromium lifecycle, bounded snapshots and artifacts |
| `{"tool":"browser.action",…}` | BrowserInteract (+ FileRead for uploads) | one typed ref action; high-impact effects pause for live approval |
...
| Backend | Role |
| --- | --- |
| **model-router** | BYOK provider adapters; constitution/workspace/catalog prefix sections for caching; media parts when catalog allows. Catalog tool-calling models use HTTP tools then JSON fallback. `grok-build/*` stays JSON-only through an isolated `grok` CLI. |
