...
| Backend | Role |
| --- | --- |
| **model-router** | BYOK provider adapters; prompt segments for caching; media parts when catalog allows. Catalog tool-calling models use HTTP tools then JSON fallback. `grok-build/*` stays JSON-only through an isolated `grok` CLI. |
