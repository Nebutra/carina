...
- Optional `-gateway-workspace` binds session-bearing Gateway work to one existing directory; Unix socket stays unpinned. Not ACP, not multi-tenant SaaS.
...
- The same optional workspace pin applies; a foreign `workspace_root` is forbidden
