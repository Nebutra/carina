// carina-worker joins a Carina daemon over TCP and offers itself as an
...
func main() {
	...
	if *server == "" {
		fmt.Fprintln(os.Stderr, "usage: carina-worker --server <host:port> [--name N] [--kind remote|ci|sandbox]")
		os.Exit(2)
	}
	...
	if err := client.Call("worker.register", map[string]any{"name": *name, "kind": *kind}, &reg); err != nil {
		log.Fatalf("carina-worker: register: %v", err)
	}
	fmt.Printf("carina-worker %q (%s) joined %s as %s\n", *name, *kind, *server, reg.WorkerID)
