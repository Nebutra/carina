...
Carina 仍处于 alpha，但已经具备 fail-closed 的公开发布流水线。`0.10.0` 提供完成签名并通过 Apple notarization 的 macOS 包、Linux 归档与系统包、带 provenance 的 npm launcher 和原生平台包、Windows worker，以及已打包的 VS Code、Web Harness 和 macOS/Linux Tauri Harness 客户端。用户可以通过脚本、Homebrew、npm 或 release 归档安装；源码构建继续面向贡献者和运行时集成者。
...
- **Durable job 控制**：后台任务句柄、按 lineage 隔离的等待/取消和仅摘要结果。
- **只读仓库证据**：typed、bounded 的 Git status、diff、log 工具。
- **受治理的浏览器自动化**：隔离的原生 Chromium、结构化 snapshot、typed action 和隔离 artifact。
- **默认关闭的 Proactive**：只读证据、canary 控制和可测量的前台开销。
...
| Context engine | 原生 `auto` / `off` / `noop` 边界、本地诊断，不内置外部压缩运行时 |
...
- Windows arm64/x64 worker、VS Code VSIX、Web Harness 归档，以及 macOS/Linux 原生 Tauri Harness 安装包；
- `Nebutra/tap/carina` Homebrew Formula。

仍需外部激活或保留的 alpha 限制：

- 公共容器 registry 发布；
- 已打包 VS Code/Web Harness 客户端的 Marketplace 与托管发布；
...
Operator bundle 都在 [`v0.10.0` release](https://github.com/Nebutra/carina/releases/tag/v0.10.0)。
...
在交互终端运行裸 `carina`（可选 `-session` / `-workspace` 等 flag）。任务提交等待 daemon
