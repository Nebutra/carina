...
func TestTypedParityAndEventSubscription(t *testing.T) {
	...
	if CompatibleRuntimeVersion != "0.8.1" {
	...
	go func() {
		...
		for len(seen) < 4 {
			...
			case "runtime.initialize":
				result = map[string]any{"runtime_version": "0.8.1", "protocol_version": "1.2.0", "projection_version": "1.0.0", "capabilities": map[string]any{"tool_call_lifecycle": true, "event_schema_version": "0.3.0"}}
			case "session.create":
